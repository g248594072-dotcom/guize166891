...
${formData.characters ? formData.characters.map((c: { name: string; desc: string }, i: number) => `,\n  { "op": "replace", "path": "/角色档案/CHR-${String(i + 1).padStart(3, '0')}", "value": { "姓名": "${c.name}", "状态": "出场中", "描写": "${c.desc}", "当前内心想法": "", "性格": [], "性癖": [], "敏感部位": [], "隐藏性癖": "", "身体信息": { "年龄": 18, "身高": 165, "体重": 50, "三围": "B86 W58 H88", "体质特征": "普通" }, "数值": { "好感度": 30, "发情值": 20, "性癖开发值": 10 }, "当前综合生理描述": "" } }`).join('') : ''}
]
</JSONPatch>
</UpdateVariable>`;
}
